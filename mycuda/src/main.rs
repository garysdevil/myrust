...
fn test(ptx: CString) -> Result<(), Box<dyn Error>> {
    ...
    // 为数据创建buffers
    let mut in_x = DeviceBuffer::from_slice(&[1.0f32; 1000000])?;
    let mut in_y = DeviceBuffer::from_slice(&[2.0f32; 1000000])?;
    let mut out_1 = DeviceBuffer::from_slice(&[0.0f32; 1000000])?;
    ...
    // 将GPU计算结果拷贝回主机内存
    let mut out_host = [0.0f32; 2000000];
    out_1.copy_to(&mut out_host[0..1000000])?;
    out_2.copy_to(&mut out_host[1000000..2000000])?;
